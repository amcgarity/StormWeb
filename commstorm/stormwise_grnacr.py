...
def stormwise(amplPath,inYamlDoc,benefitDict,budget,fDict,kjdict,kdict,Kgdict,Krdict):
    amplDat = generate_ampl_dat_file(inYamlDoc,fDict,kjdict,kdict,Kgdict,Krdict)
    # store the structure of the problem as found in the YAML file
    with open('stormwise_grnacr_com.dat', 'w') as fout:     
        fout.write(amplDat)
        fout.close()

    amplBenefits = generate_ampl_benefit_file(benefitDict,budget)
    with open('stormwise_grnacr_com_benefits.dat', 'w') as fout:
...
def evaluate_solution(decisions,s,inYamlDoc,kdict):
    solutionDict = {}
